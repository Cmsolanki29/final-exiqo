...
const GNNTrainingPanel = () => {
  ...
  // Backend returns last_run.completed_at (nested), not top-level trained_at
  const trainedAt =
  ...
  // Backend returns embeddings.total (nested) and last_run.num_users
  const graphUsers =
    status?.last_run?.num_users ??
    status?.graph_users ??
    status?.users_in_graph ??
    null;

  const embeddings =
    status?.embeddings?.total ??
            ...
            value={
              <span className="inline-flex items-center gap-1.5">
                <span
                  ...
                  style={{
                    background:
                      (health?.enabled ?? health?.feature_flag_enabled)
                        ? "#22c55e"
                        : "#94a3b8",
                  }}
                />
                {(health?.enabled ?? health?.feature_flag_enabled) ? "Enabled" : "Disabled"}
              </span>
            }
          />
