    ...
    <>
      <div className={`flex flex-wrap gap-2 ${compact ? "mb-3" : "preset-wrap mb-3"}`}>
        {presets.map((label) => (
          <button
            key={label}
            type="button"
            className="chip-btn min-h-[48px] rounded-full border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-medium text-gray-300 transition hover:bg-white/[0.08] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/60 md:min-h-0"
      ...
      <div className={`flex flex-col gap-2 ${compact ? "sm:flex-row sm:items-stretch" : "sim-input-row"}`}>
        ...
        <input
          ...
          className="min-h-[48px] flex-1 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white placeholder:text-gray-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400/60"
        />
      ...
      {loading ? (
        <div className="mt-3">
          <p className="mb-2 text-xs text-gray-400">Running scenario against your profile…</p>
      ...
      {result && !loading ? (
        ...
        >
          <div>
            <h4 className="text-xs font-bold uppercase tracking-wide text-gray-400">Scenario</h4>
          ...
          {(result.alternatives || []).length > 0 && (
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-1">Smarter alternatives</p>
          ...
          {result.computed_from === "real_transactions" && (
            <p className="text-[10px] text-gray-600 text-right">Computed from your real transaction history</p>
          )}
        </motion.article>
      ) : null}
    </>
  ...
  if (compact) {
    return (
      <GlassCard padding="sm" surface="panel" className="border-white/[0.08] lg:pb-24">
        ...
        <p className="mt-1 text-xs text-gray-400">Model a purchase or lifestyle change.</p>
